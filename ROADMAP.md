...
- The local poudriere build fetches dependencies from the upstream FreeBSD binary package repository when poudriere considers that acceptable. The aarch64 path currently uses `quarterly` plus a temporary 2026Q2 `PORTS_REF` pin so `lang/rust` and its dependency closure are fetched instead of built. Revisit this pin as the upstream package set moves.
