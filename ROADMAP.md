...
- The dedicated builder target is now FreeBSD/amd64 with roughly 24 GiB RAM and 1-1.5 TiB mixed SSD storage. FreeBSD hosts poudriere directly; Linux becomes a guest for Linux-centric workflows.
- The native FreeBSD builder package bootstrap is scripted, and the host readiness check validates poudriere, hut, pkg, sudo, rsync, git, curl, and cross-build support. Convert the remaining storage layout into real IaC once the final pool/dataset names are known.
- The local QEMU workstation fallback still depends on distro package names such as `qemu-full`, `edk2-ovmf`, `edk2-aarch64`, `cdrtools`, `openssh`, `openbsd-netcat`, `rsync`, `hut`, `curl`, and `xz`.
