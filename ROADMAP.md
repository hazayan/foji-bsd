...
- The local builder host currently depends on distro package names such as `qemu-full`, `edk2-ovmf`, `edk2-aarch64`, `cdrtools`, `openssh`, `openbsd-netcat`, `rsync`, `github-cli`, `curl`, and `xz`. Convert these observed package requirements into IaC once the dedicated build machine target distribution is fixed.
