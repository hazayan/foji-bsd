...
- Poudriere state is cached with GitHub Actions cache under `poudriere-cache`, including jail, ports, package, distfile, and build cache state. This avoids repeatedly updating base and rebuilding heavy unchanged packages, but it makes GitHub cache state part of the build pipeline. `POUDRIERE_CACHE_EPOCH` can be bumped to abandon stale caches. Revisit with cache size controls, periodic clean rebuilds, and clearer cache deletion automation.
